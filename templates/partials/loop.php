<?php if ( $query->have_posts() ) : ?>
 ...
 <?php endwhile; ?>
	<?php if ( 'load more' === $attributes['pagination'] ) : ?>
		<?php if ( ! $this->is_last_page( $query ) ) : ?>
            <div class="ajax-posts__load-more">
                <button class="js-load-more"  data-page="<?= $this->get_page_number($query) + 1; ?>">
					<?php esc_html_e( 'Load more', 'ajax-filter-posts' ); ?>
                </button>
            </div>
		<?php endif; ?>
	<?php else : ?>
        <div class="ajax-posts__pagination">
			<?php
			$big         = 999999999;
			$list_string = paginate_links(
				array(
                    //'base'      => str_replace( $big, '%#%', esc_url( get_pagenum_link( $big ) ) ), // The URL changes to https://domain.com/wp-admin/admin-ajax.php?paged=3
                    //'base'      => get_pagenum_link( 1 ) . '%_%', // The URL changes to https://domain.com/wp-admin/admin-ajax.php?paged=3
					'base'      => '%_%',
					'format'    => '?paged=%#%',
                    //'current'   => max( 1, get_query_var( 'paged' ) ), // If I use this option it not updating pagination.
					'current'   => $this->get_page_number( $query ),
					'total'     => $query->max_num_pages,
					'prev_text' => __( '«' ),
					'next_text' => __( '»' ),
				)
			);
			$links = str_replace( '<a ', '<a class="page-numbers js-load-mores-x" ', $list_string );
